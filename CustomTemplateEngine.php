...
class CustomTemplateEngine extends \ExternalModules\AbstractExternalModule
{
    ...
    */
    ...
    public function saveFileToField($filename, $file_contents, $field_name, $record, $event_id)
    {
        ...
        return $docs_id; // falsy 0 if could not upload file
    }
    ...
    public function uploadImages()
    {
        ...
        $token = filter_input(INPUT_POST, 'ckCsrfToken', FILTER_SANITIZE_SPECIAL_CHARS);

        // $cookie_token = $_COOKIE["ckCsrfToken"];
        $cookie_token = filter_input(INPUT_COOKIE, 'ckCsrfToken', FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        if ($token === $cookie_token)
        {
            ...
            // Check the $_FILES array and save the file. Assign the correct path to a variable ($url).
                ...
                {
                    $message = "ERROR: error uploading file, error is: " . $upload['error'] . ".";
                }
            }
            else
            {
                $message = "ERROR: error uploading file, file does not exist or is empty.";
            }
        }
        else
        {
     ...
     *
     * Retrieves body, header, and footer contents of template passed via HTTP wPOST.
    ...
    private function getCustomRecordLabelsSecondaryFieldAllRecords($records=array(), $removeHtml=false, $arm=null, $boldSecondaryPkValue=false, $cssClass='crl')
	{
		...
		if ($secondary_pk != '')
		{
            ...
            $query->add("select record, event_id, value from " . $this->redcap_data_table . " where project_id = ? and field_name = ?", [$this->pid, $secondary_pk]);
        ...
        <div class="container"> 
            <div class="jumbotron">
                ...
                <?php if ($rights[$this->userid]["reports"]) :?>
                    <div class="container syntax-rule">
                        <a class="btn btn-link" style="font-size: 16px; text-decoration: underline; font-color: blue;" href=<?php print $this->getUrl("CreateTemplate.php");?>>Create New Template</a> |
                        ...
                        <table class="table" style="width:100%;">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="data">
                                        <input id="applyFilter" type="checkbox" <?php print filter_input(INPUT_GET, 'filter', FILTER_SANITIZE_SPECIAL_CHARS) == "1" ? "checked" : ""; ?>>
                ...
                if ($("#participantIDs").val().length > 1)
                {
                    ...
                    fileDownloadCheckTimer = window.setInterval(function () {
                        ...
                        if (cookieValue == token)
                        {
                            ...
                            $("#participantIDs").selectpicker("deselectAll");  // bootstrap-select change for v1.14.0-beta3 2023-08-16 by Dan Evans
                            $("#fill-template-btn").text("Fill Template");
                        }
                    }, 1000)
                }
    ...
    {
        if (!empty($_GET['pid']) && is_numeric($_GET['pid'])) {  // engineer around calling outside project context
    ...
    protected function page_top($project_id, $record, $instrument, $event_id, $group_id, $repeat_instance, $is_survey=false, $survey_hash=null, $response_id=null) {
        ...
        foreach (array_keys($Proj->forms[$instrument]['fields']) as $f) {
            if ($Proj->metadata[$f]['element_type']==='file' && strpos($Proj->metadata[$f]['misc'], static::FILEUPLOAD_FIELD_TAG)!==false) {
                $ff = true;
                break;
            }
        }
        ...
        $templateSelect = '<select class="upload-source-select form-select form-select-sm"><option value="0">Browse and select file</option>';
        $n = 0;
        foreach($valid_templates as $template) {
            $n++;
    ...
    public function fillAndSave() {
        ...
        $instance = htmlentities($_REQUEST['instance'], ENT_QUOTES);
        $field_name = explode('-', htmlentities($_REQUEST['field_name'], ENT_QUOTES))[0];
        $template_name = htmlentities($_REQUEST['template_name'], ENT_QUOTES);

        $save_filename = $template_name.'_'.$record.'_'.date('Y-m-d_His');
        ...
        else
        {
            ...
            $file_delete_page   = APP_PATH_WEBROOT . "DataEntry/file_delete.php?pid=$project_id&page=" . htmlentities($_GET['instrument'], ENT_QUOTES);
        }
